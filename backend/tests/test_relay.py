...
os.environ.setdefault("SENDER_EMAIL", "vasanth@example.com")
...
def test_message_is_delivered(client, provider):
    response = client.post(
        "/api/message",
        json={"message": "I really didn't expect this...", "sender_name": "Vasanth"},
    )
...
def test_email_contents(client, provider):
    client.post("/api/message", json={"message": "line one\nline two", "sender_name": "Vasanth"})
    sent = provider.sent[0]
    assert sent["subject"] == "A little message from Vasanth"
    assert "line one" in sent["text_body"]
    assert "line two" in sent["text_body"]
    assert "Vasanth just sent you a message" in sent["text_body"]
    assert "Sent from the Vasanth Surprise CLI" in sent["text_body"]
...
def test_client_field_is_optional(client, provider):
    ...
    assert client.post(
        "/api/message", json={"message": "hi", "client": "vasanth-cli/1.0.0"}
    ).status_code == 200
...
def test_recipient_cannot_be_overridden_even_when_request_is_otherwise_valid(client, provider):
    client.post("/api/message", json={"message": "hello", "sender_name": "Vasanth"})
...
def test_header_injection_via_sender_name_is_neutralised(client, provider):
    client.post(
        "/api/message",
        json={"message": "hi", "sender_name": "Vasanth\r\nBcc: attacker@example.com"},
    )
...
def test_template_renders_a_timestamp():
    text = email_template.render_text("hi", "Vasanth")
    assert "UTC" in text
    assert email_template.render_subject("Vasanth") == "A little message from Vasanth"
