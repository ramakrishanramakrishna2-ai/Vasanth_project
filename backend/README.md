...
A single-endpoint backend that receives one message from the `vasanth` CLI and
...
```
vasanth CLI  ──HTTPS──▶  this service  ──API──▶  email provider  ──▶  your inbox
```
...
SENDER_EMAIL=vasanth@your-verified-domain.com
SENDER_NAME=Vasanth Surprise CLI
```
...
SENDER_EMAIL=vasanth@example.com \
...
```bash
docker build -t vasanth-relay .
docker run -p 8000:8000 --env-file .env vasanth-relay
```

After deploying, confirm configuration and then set the URL in
`src/vasanth/api.py` as `DEFAULT_ENDPOINT`:
