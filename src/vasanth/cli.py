...
def scene_send() -> None:
    ...
    while True:
        ...
        # Confirmation screen, with the privacy notice he sees every time.
        anim.transition(0.35)
        ui.blank(2)
        _preview(message)
        ui.blank(2)
...
def run(argv=None) -> int:
    ...
    if "--version" in argv or "-V" in argv:
        ...
        print("vasanth %s" % __version__)
        return 0

    if "--help" in argv or "-h" in argv:
        print("vasanth -- a small terminal experience.\n")
        print("Usage: vasanth [--skip-intro] [--fast] [--version]\n")
