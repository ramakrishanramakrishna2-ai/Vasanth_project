...
secret trigger, so a broken scene fails here rather than in front of him.
"""
...
from vasanth import animation as anim
from vasanth import api, cli
from vasanth import messages as M
...
def test_version_and_help_do_not_start_the_experience(capsys):
    ...
    assert "vasanth" in capsys.readouterr().out
...
def sent_ok(monkeypatch):
    ...
    def fake(message, sender_name="Vasanth"):
